...
<div class="header">
    ...
    // $product = Illuminate\Support\Facades\DB::table('products')->where('min_qty', '<', 10)->get();
      ...
      <li class="nav-item dropdown">
        <a href="javascript:void(0);" class="dropdown-toggle nav-link" data-bs-toggle="dropdown">
          ...
          <span class="badge rounded-pill">{{ $emitoday_count}}</span>
        </a>
                  ...
                  <div class="media d-flex">
                    ...
                    <div class="media-body flex-grow-1">
                      <p class="noti-details"><span class="noti-title">
                          ...
                          {{ $key+1 }}) Date: <code>({{ $item->emi_date }})</code> | Name:
                          <code>({{ $cName->name }})</code> | Invoice: <code>{{ $item->ref_code }}</code> | EMI:
                          <code>{{ $item->emi_amount }}</code></p>
  ...
  <div class="dropdown mobile-user-menu">
    ...
    <div class="dropdown-menu dropdown-menu-right">
      ...
      <form action="{{ route('logout') }}" method="post">
        @csrf
        <button type="submit" class="dropdown-item logout pb-0"><img src="{{asset('backend')}}/img/icons/log-out.svg"
            class="me-2" alt="img">{{ trans('header.header.dropdown.logout') }}</a>
      </form>
    </div>
  </div>
  <!-- /Mobile Menu -->
